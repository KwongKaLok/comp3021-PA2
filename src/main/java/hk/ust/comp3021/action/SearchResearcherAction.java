...
public class SearchResearcherAction extends Action {
  public enum SearchResearcherKind {
    PAPER_WITHIN_YEAR, JOURNAL_PUBLISH_TIMES, KEYWORD_SIMILARITY,
  };
  ...
  public SearchResearcherAction(String id, User user, Date time, String searchFactorX, String searchFactorY,
      SearchResearcherKind kind) {
  ...
  /**
   * TODO `searchFunc1` indicates the first searching criterion i.e., Search
   * researchers who publish papers more than or equal to X times in the recent Y
   * years
   * @param null
  ...
  /**
   * TODO `searchFunc2` indicates the second searching criterion, i.e., Search
   * researchers whose papers published in the journal X have abstracts of which
   * the length is more than or equal to Y.
   * @param null
   * @return `actionResult` that contains the relevant researchers
   */
  ...
  /**
   * TODO `searchFunc3` indicates the third searching criterion i.e., Search
   * researchers whoes keywords have more than or equal to similarity X% as one of
   * those of the researcher Y.
   * @param null
   * @return `actionResult` that contains the relevant researchers PS: 1) In this
   *         method, you are required to implement an extra method that calculates
   *         the Levenshtein Distance for two strings S1 and S2, i.e., the edit
   *         distance. Based on the Levenshtein Distance, you should calculate
   *         their similarity like `(1 - levenshteinDistance / max(S1.length,
   *         S2.length)) * 100`. 2) Note that we need to remove paper(s) from the
   *         paper list of whoever are co-authors with the given researcher.
   */
